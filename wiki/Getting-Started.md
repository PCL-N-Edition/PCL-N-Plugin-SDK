...
> Applies to PCL N Plugin SDK 0.2.5 · Plugins run inside **PCL.Plugin.Sidecar** (CoreCLR); desktop shell stays AOT. See [Architecture and Boundaries](Architecture-and-Boundaries).

> 适用于 SDK `0.2.5`。本页从空目录开始创建一个可以打包为 `.pnp` 的插件。
...
dotnet add package PCLN.Plugin.Abstractions --version 0.2.5
dotnet add package PCLN.Plugin.Sdk --version 0.2.5
dotnet add package PCLN.Plugin.Analyzers --version 0.2.5
dotnet add package PCLN.Plugin.Sdk.Build --version 0.2.5
```
...
```text
bin/Release/net10.0/dev.example.hello-0.2.5.pnp
```
